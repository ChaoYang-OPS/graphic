...
"""
    程序在运行期间产生异常时，为了让程序能够继续执行，可以对抛出的异常实例对象进行捕获和处理，
这是通过try-except语句实现的: 把可能会产生异常的代码放在try语句块中以捕获异常实例对象，
...
"""
    如果抛出的异常实例对象所对应的类对象是except子句中的异常类对象的子类，那么该except子句也会
被匹配。
"""
...
"""
    当多个异常类对象对应的异常处理代码完全相同时，可以把这些异常类对象以元组的形式放在一个
except子句中。
"""
...
"""
    如果想在匹配到的except子句中访问异常实例对象，可以在except子句中的冒号前添加关键字as和
一个变量。
"""
...
"""
    为了在except语句中将所有的异常类对象尽可能地覆盖到，可以将最后一个except子句中的
