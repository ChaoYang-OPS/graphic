...
同时，这个类对象的实例对象被称为上下文管理器。
    with语句会让上下文管理器创建一个运行时上下文，当进入运行时上下文件时自动调用特殊方法__enter__()，
当离开运行时上下文时自动调用特殊方法__exit__()。
    with语句的语法格式:
with 上下文表达式 [as 变量]:
    with语句体
...
class MyContextManager(object):
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        print('异常的错误信息:{}'.format(exc_val))
        print('异常调用堆栈的跟踪信息:{}'.format(exc_tb))
