...
仍然可以通过这个变量调用方法，在程序运行过程中根据变量所引用对象的类型，动态地决定调用哪个对象中的方法。
...
"""
    Python是动态语言，在调用函数时不会检查参数类型，从而导致与静态语言（例如Java）的多态
是有区别的。对于静态语言，实现多态有三个必要条件：
1. 继承
2. 重写
3. 父类类型的变量引用父类或子类类型的实例对象
    因此。对于静态语言，在上面的程序中，要限定形参parent的类型是ParentClass,传入的实参只能是ParentClass、
ChildClass1和ChildClass2的实例对象。
