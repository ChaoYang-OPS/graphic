...
为了让内函数接收任意类型的参数，将内函数的形参定义为(*args, **kwargs)。在内函数中，首先完成
为被装饰函数添加的新功能，然后调用被装饰的函数。
    把装饰器应用到被装饰函数的语法为:在被装饰函数的前面添加"@装饰器的函数名"。在被装饰函数add
的前面添加@log后，相当于执行了语句: add = log(add), 首先，被装饰的函数add会作为实参传递给装饰器log,
然后，返回装饰器的内函数wrapper，最后，将内函数wrapper赋值给名为add(被装饰函数的函数名)的变量，
...
def log(func):
    def wrapper(*args, **kwargs):
        print("函数{}被调用了".format(func.__name__))
...
其中functools.wraps指的是标准库模块functools中的函数wraps。
"""
import functools
...
def log3(month, day):
    def decorator(func):
        @functools.wraps(func)
        def wrapper(*args, **kwargs):
            print("{}{}函数{}被调用了".format(month, day, func.__name__))
            return func(*args, **kwargs)
