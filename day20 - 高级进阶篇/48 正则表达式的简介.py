...
"""
    正则表达式是字符串处理的利器，它可以用于字符串的匹配、查找、替换、和劈分。虽然在前面的课程中也学习过
字符串的查找、替换和劈分，但是借助正则表达式和标准库模块re可以实现更加强大的查找、替换和劈分。
其中，标准库模块re是专门用于支持正则表达式的。
    正则表达式是一个特殊的字符串，该字符串中包含了一些特定字符，这些特定字符都有明确的匹配规则，
因此，正则表达式是一个定义了匹配规则的字符串。如果某个字符串符合正则表达式定义的所有匹配规则，那么该字符串就与该正则表达式匹配。
