...
"""
    可以用于for-in语句的对象被称为可迭代对象(Iterable)对象。例如: range、list、元组、字符串、
字典、集合、生成器、都是可迭代对象。
    可以调用内置函数isinstance()判断一个对象是否是可迭代对象。标准库模块collections中的类
Iterable用于表示可迭代对象。
"""
...
"""
    如果一个可迭代对象作为内置函数next()的实参从而支持惰性计算，那么该对象被称为
迭代器(Iterator)对象。
...
"""
    如果一个对象同时实现了特殊方法__iter__()和__next__()，那么该对象也称为迭代器对象。
如果，将该对象用于for-in语句，for-in语句首先会调用特殊方法__iter__()返回一个可迭代对象，然后
不断调用该可迭代对象特殊方法__next__()返回下一次迭代的值，直到遇到StopIteration时退出循环。
"""
