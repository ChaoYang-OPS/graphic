...
class ScoreBoard:
    ...
    def draw_current_score(self):
        ...
        # 获取当前得分的文本的矩形
        self.current_score_rect = current_score_text_surface.get_rect()
        ...
        # 在窗口的的左上角绘制当前得分
        self.window.blit(current_score_text_surface, self.current_score_rect)

    def draw_highest_score(self):
        """在得分板中绘制最高得分"""
        # 最高得分的文本
        highest_score_text = "最高得分:{}".format(self.highest_score)
        # 获得最高得分的文本对应的surface对象
        highest_score_text_surface = self.font_36.render(highest_score_text,
                                                         True,
                                                         constants.WHITE_COLOR)
        # 获取最高得分的文本的矩形
        self.highest_score_rect = highest_score_text_surface.get_rect()

        # 将提示最高得分的文本矩形定位在窗口的左上角当前得分的下面
        self.highest_score_rect.left = constants.MARGIN
        self.highest_score_rect.top = self.current_score_rect.bottom + constants.MARGIN

        # 在窗口的的左上角当前得分的下面绘制最高得分
        self.window.blit(highest_score_text_surface, self.highest_score_rect)

    def draw_current_level(self):
        """在得分板中绘制当前关数"""
        # 当前关数的文本
        current_level_text = "当前关数:{}".format(self.current_level)
        # 获得当前关数的文本对应的surface对象
        ...
        # 将提示当前关数的文本矩形定位在窗口的左上角最高得分的下面
        current_level_rect.left = constants.MARGIN
        current_level_rect.top = self.highest_score_rect.bottom + constants.MARGIN

        # 在窗口的的左上角最高得分的下面绘制当前关数
    ...
    def update_current_level(self):
        ...
        if self.current_score > constants.LEVEL1_SCORE_MAX \
        ...
        elif self.current_score > constants.LEVEL2_SCORE_MAX \
                and self.current_level == 2:
            # 将当前关数设置为3
            self.current_level = 3

        # 如果当前得分大于关数3的得分最大值，并且当前关数是3
        elif self.current_score > constants.LEVEL3_SCORE_MAX \
        ...
        elif self.current_score > constants.LEVEL4_SCORE_MAX \
                and self.current_level == 4:
            # 将当前关数设置为5
